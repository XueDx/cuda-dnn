#ifndef LINEARDECODER_H
#define LINEARDECODER_H
...
public:
    ...
    LinearDecoder(int visible, int hidden) : hiddenSize(hidden), visibleSize(visible) {
...
void  LinearDecoder::train(float* d_data, int trainSize, int maxIter){
	...
	for(int iteration = 1; iteration <= maxIter; iteration++){
		...
		*******************************************************************************/
		...
		d_a3 = d_z3; // We don't use d_z3 anywhere else, so we don't need to copy it to d_a3
		...
		//Calculate d3
		mSub<<<ceilf(visibleSize*trainSize/(float)NTHREADS), NTHREADS>>>(d_a3, d_data, d_d3, visibleSize, trainSize);
...
float*  LinearDecoder::feature(float* d_data, int trainSize){
	cublasHandle_t handle;
	float* d_a2;
	float* d_ones_tx1;
	...
	Logistic<<<ceilf(hiddenSize*trainSize/(float)NTHREADS),NTHREADS>>>(d_a2,d_a2,hiddenSize*trainSize); // d_a2 <- Logistic(d_z2)m
